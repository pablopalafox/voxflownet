...
class VoxelGrid(Structure):
    ...
    def __init__(self, *, points, sceneflow, n_x=1, n_y=1, n_z=1,
                 xyzmin=np.array([-20.0, -20.0, 0.0]), xyzmax=np.array([20.0, 20.0, 40.0]),
                 compute_sceneflow=False,
    ...
    def compute(self):
        ...
        # Get the number of voxels in the 3D voxel grid
        ...
        self.voxel_x = np.clip(np.searchsorted(self.segments[0], self._points[:, 0]) - 1, 0, self.x_y_z[0]) # x cooord
        self.voxel_y = np.clip(np.searchsorted(self.segments[1], self._points[:, 1]) - 1, 0, self.x_y_z[1]) # y cooord
        self.voxel_z = np.clip(np.searchsorted(self.segments[2], self._points[:, 2]) - 1, 0, self.x_y_z[2]) # z cooord
        self.voxel_n = np.ravel_multi_index([self.voxel_x, self.voxel_y, self.voxel_z], self.x_y_z)

        # NOTE:
        # ''self.voxel_x'' is an array of length equal to the number of points in the point cloud
        # where each element indicates at what index of the x axis the point is situated
        # NOTE:
        # ''self.voxel_n'' is an array of length equal to the number of points in the point cloud
        # where each element indicates the index of the point in the grid in a flattened vector
        # This flattened vector results from flattening the 3D voxel grid coordinates (0, 0, 0) into a single array

        # The indices within self.voxel_n range from 0 to ´self.n_voxel´
        ...
        # Compute SCENE FLOW (i.e., compute motion vector for each voxel)
        if self.compute_sceneflow:
            self.voxel_sf = np.zeros((self.n_voxels, 3))
            # Now get a vector of length at most the number of voxels,
            # where every element encodes the number of points in every voxel
            count = np.bincount(self.voxel_n)
            # Go through self.voxel_n and compute a motion vector for every voxel
            for point_idx, voxel_idx in enumerate(self.voxel_n):
                # if np.array_equal(self.voxel_sf[voxel_idx], np.array([0., 0., 0.])):
                #     self.voxel_sf[voxel_idx] = self._sceneflow[point_idx]
                self.voxel_sf[voxel_idx] += (self._sceneflow[point_idx] / count[voxel_idx])
    ...
    def get_voxelgrid_sceneflow(self):
        if self.compute_sceneflow:
            return self.voxel_sf.reshape((self.x_y_z[0], self.x_y_z[1], self.x_y_z[2], -1))
        else:
            return None

    def get_feature_vector(self, mode="threshold", min_n_points_in_voxel=10):
        ...
        if mode == "binary":
            ...
            zeros[np.argwhere(vector > min_n_points_in_voxel)] = 1
            vector = zeros
